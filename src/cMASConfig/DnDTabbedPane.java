...
/**Code copied from Eugene Yokota
...
public class DnDTabbedPane extends JTabbedPane {
    ...
    /**
     * Modified by Kalle Takeuchi in order to adapt to application
     * @param a_data
     * @param a_targetIndex
     */
    private void convertTab(TabTransferData a_data, int a_targetIndex) {
    	//Cast to tabbed pane instead of DnD
        TabbedPane source = (TabbedPane) a_data.getTabbedPane();
        ...
        //Cast to tab
        Tab cmp = (Tab) source.getComponentAt(sourceIndex);
        String str = source.getTitleAt(sourceIndex);
        //Cast this as TabbedPane to use methods
        TabbedPane tpThis = (TabbedPane) this;
        
        if (this != source) {
            source.removeTabCMAS(cmp);

            if (a_targetIndex == getTabCount()) {
                tpThis.addNewTab(cmp);
            } else {
                if (a_targetIndex < 0) {
                    a_targetIndex = 0;
                } // if

                tpThis.addNewTab(cmp, a_targetIndex);
        ...
        if (a_targetIndex == getTabCount()) {
            ...
            //System.out.println("   <: press="+prev+" next="+next);
            source.removeTabCMAS(cmp);
            tpThis.addNewTab(cmp, a_targetIndex - 1);
